...
class SpatialSoftmax(torch.nn.Module):
    def __init__(self, height, width, channel, device, data_format='NCHW'):
        ...
        self.indices_r = torch.tensor(np.reshape(self.indices_r, (-1, height * width)), dtype=torch.float)
        self.indices_c = torch.tensor(np.reshape(self.indices_c, (-1, height * width)), dtype=torch.float)
    ...
    def forward(self, feature):
        ...
        feature = 100 * feature.reshape(-1, h * w)
...
class Fall_Net(torch.nn.Module):
    def __init__(self, height, width, channel, device):
        ...
        self.arg_softmax = SpatialSoftmax(64, 48, 17, device=device)
    ...
    def forward(self, x):
        ...
        pose_cor = x
        x = x.view(-1, 34).float()  # Ensure x is of type Float
        x = self.linear1(x)
        x = self.relu1(x)
        x = self.dropout1(x)
        residual_x = x
